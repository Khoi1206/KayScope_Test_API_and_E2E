  ...
  return (
    <div className="border-t border-th-border flex flex-col" style={{ height: '40%', minHeight: '180px' }}>
      ...
      <div className="flex items-center gap-3 px-4 py-2 border-b border-th-border shrink-0">
        <span className="text-xs text-th-text-3 font-medium">Response</span>
        {response && (
          <>
        ...
        {response && (['Pretty', 'Headers', 'Cookies', 'Timing', 'Raw'] as const).map(t => (
          <button key={t} onClick={() => setResponseTab(t)}
            className={`text-xs px-2 py-1 rounded transition ${responseTab === t ? 'bg-th-input text-th-text-2' : 'text-th-text-3 hover:text-th-text-2'}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-auto p-4">
        ...
        {response && responseTab === 'Timing' && requestTiming && (
          <div className="space-y-3 max-w-md">
            <p className="text-[10px] text-th-text-3 italic">* Timing breakdown is estimated from total duration</p>
            {[
              ...
              <div key={t.label} className="space-y-1">
                <div className="flex justify-between text-xs">
                  <span className="text-th-text-3">{t.label}</span>
                  <span className="text-th-text-2 font-mono">{t.value}ms</span>
                </div>
            ...
            <div className="flex justify-between text-xs pt-2 border-t border-th-border">
              <span className="text-th-text-2 font-medium">Total</span>
        ...
        {!response && !isSending && !sendError && (
          <span className="text-th-text-3 text-xs">Hit Send to see the response</span>
        )}
      </div>
    </div>
  )
