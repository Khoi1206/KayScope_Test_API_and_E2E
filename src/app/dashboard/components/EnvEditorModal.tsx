...
export function EnvEditorModal({ env, onSave, onCancel }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={onCancel}>
      <div className="bg-[#1e1e1e] border border-gray-700 rounded-xl shadow-2xl w-full max-w-lg p-5 max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <h3 className="text-sm font-semibold text-white mb-3">{env ? 'Edit Environment' : 'Create Environment'}</h3>
        <input
          autoFocus value={name} onChange={e => setName(e.target.value)}
          placeholder="Environment name"
          className="w-full bg-gray-800 border border-gray-600 rounded-md px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-orange-500 mb-3"
        />
        <div className="text-xs text-gray-500 font-medium mb-2">Variables</div>
        <div className="flex-1 overflow-y-auto space-y-1.5 mb-4">
          {vars.map((v, i) => (
            <div key={i} className="flex items-center gap-1.5 group">
              ...
              <input value={v.key} onChange={e => updateVar(i, 'key', e.target.value)} placeholder="KEY"
                className="flex-1 bg-gray-800 border border-gray-700 rounded px-2 py-1.5 text-gray-300 text-xs placeholder-gray-600 focus:outline-none focus:border-orange-500 font-mono" />
              <input value={v.value} onChange={e => updateVar(i, 'value', e.target.value)} placeholder="value"
                type={v.secret ? 'password' : 'text'}
                className="flex-1 bg-gray-800 border border-gray-700 rounded px-2 py-1.5 text-gray-300 text-xs placeholder-gray-600 focus:outline-none focus:border-orange-500 font-mono" />
              <button onClick={() => updateVar(i, 'secret', !v.secret)} className={`shrink-0 text-xs px-1 ${v.secret ? 'text-orange-400' : 'text-gray-600'}`} title="Toggle secret">
          ...
          <button onClick={addVar} className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-300 mt-1">
